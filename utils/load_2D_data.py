        ...
        for i, scan_name in enumerate(train_list):    
            try:    
                ...
                with np.load(path_to_np, allow_pickle=True) as data:    
                    ...
                    mask_patches = data['mask_patches']    
                    positions = data['positions']    
                    patch_sizes_data = data['patch_sizes']  # Renombrado para evitar confusión  
                    original_shape = data['original_shape']    
            except:    
                logging.info('\nPre-made numpy array not found for {}.\nCreating now...'.format(scan_name[:-4]))    
            ...
            # SOLUCIÓN 2: Añadir debugging para verificar datos  
            logging.info(f'Total patches loaded: {len(patches)}')  
            logging.info(f'Patch sizes in data: {set(patch_sizes_data)}')  
                
            # Crear índices balanceados por tamaño usando los datos cargados  
            size_indices = {size: [] for size in set(patch_sizes_data)}    
            for idx, size in enumerate(patch_sizes_data):    
                size_indices[size].append(idx)    
              
            # Log de índices por tamaño  
            for size, indices in size_indices.items():  
                logging.info(f'Size {size}: {len(indices)} patches available')  
                
            # Seleccionar patches según la distribución deseada    
            selected_indices = []    
            for size_idx, size in enumerate(patch_sizes):    
                if size in size_indices and len(size_indices[size]) > 0:    
                    num_samples = max(1, int(len(patches) * size_distribution[size_idx]))  # Asegurar al menos 1  
                    if num_samples > 0:    
                        sampled = np.random.choice(size_indices[size],     
                                                 min(num_samples, len(size_indices[size])),     
                                                 replace=False)    
                        selected_indices.extend(sampled)  
                        logging.info(f'Selected {len(sampled)} patches of size {size}')  
              
            # SOLUCIÓN 2: Log de patches seleccionados  
            logging.info(f'Selected indices: {len(selected_indices)} total patches')  
              
            # Si no hay patches válidos, continuar con la siguiente imagen  
            if len(selected_indices) == 0:  
                logging.warning(f'No valid patches found for {scan_name}, skipping...')  
                continue  
            ...
            for patch_idx in selected_indices:    
                ...
                # SOLUCIÓN 3: Verificación mejorada de máscaras  
                if not np.any(mask_patch):    
                    logging.debug(f'Skipping patch {patch_idx} - empty mask')  
                    continue    
                  
                # Verificación adicional de dimensiones  
                if patch.shape[:2] != (512, 512) or mask_patch.shape[:2] != (512, 512):  
                    logging.warning(f'Patch {patch_idx} has incorrect dimensions: {patch.shape}, {mask_patch.shape}')  
                    continue  
