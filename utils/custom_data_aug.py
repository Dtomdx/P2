...
def split_image_into_patches(image, patch_size=512, overlap=0):  
    """  
    Divide una imagen en patches de tamaño fijo sin overlap  
    Retorna patches y sus posiciones para reconstrucción  
    """  
    h, w = image.shape[:2]  
    patches = []  
    positions = []  
...
def reconstruct_from_patches(patches, positions, original_shape):  
    """  
    Reconstruye imagen original desde patches usando posiciones guardadas  
    """  
    reconstructed = np.zeros(original_shape, dtype=patches[0].dtype)  
      
    for patch, (y_start, y_end, x_start, x_end) in zip(patches, positions):  
